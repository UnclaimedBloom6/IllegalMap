...
register("tick", () => {
	showingFirst = peekRoomNames.isKeyDown() && !settings.showSecrets ? true : false
	showingSecond = peekRoomNames.isKeyDown() && Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) && !settings.showAllRooms ? true : false
})
...
register("renderOverlay", () => {
			...
			for (j in dungeonMap[i]) {
				if (dungeonMap[i][j] instanceof Room) {
					if (dungeonMap[i][j].isLarge) {
						...
						if ((dungeonMap[i][j].roomType == "puzzle" || dungeonMap[i][j].roomType == "trap") && (settings.showImportantRooms || showingFirst)) { toDrawLater.push([dungeonMap[i][j].roomName, (settings.mapX + i*ms) * (10/ms) + ms, (settings.mapY + j*ms) * (10/ms)]) }
						else if (dungeonMap[i][j].roomType == "normal" && dungeonMap[i][j].roomName !== "Unknown") {
							let a = ""
							if (settings.showSecrets || showingFirst) { toDrawLater.push([`&7${dungeonMap[i][j].secrets}`, (settings.mapX + i*ms) * (10/ms) - ms+1, (settings.mapY + j*ms) * (10/ms)-ms+1]) }
							if (settings.showNewRooms) { if (dungeonMap[i][j].roomName.startsWith("NEW")) { a += ` NEW` }}
							if (settings.showAllRooms || showingSecond) { a += ` ${dungeonMap[i][j].roomName.replace("NEW ", "")}` }
							toDrawLater.push([a, (settings.mapX + i*ms) * (10/ms) + ms, (settings.mapY + j*ms) * (10/ms)])
						}
						if (settings.checkmarks !== 0) {
							if (dungeonMap[i][j].checkmark == "green") { checks.push([settings.checkmarkStyle == 1 ? greenCheck : greenCheck2, settings.mapX + (i*ms)-ms/2, settings.mapY + (j*ms)-ms/2, 2*ms, 2*ms]) }
							if (dungeonMap[i][j].checkmark == "white") { checks.push([settings.checkmarkStyle == 1 ? whiteCheck : whiteCheck2, settings.mapX + (i*ms)-ms/2, settings.mapY + (j*ms)-ms/2, 2*ms, 2*ms]) }
							if (dungeonMap[i][j].checkmark == "failed") { checks.push([settings.checkmarkStyle == 1 ? failedRoom : failedRoom2, settings.mapX + (i*ms)-ms/2, settings.mapY + (j*ms)-ms/2, 2*ms, 2*ms]) }
						}
					}
					else {
						if (dungeonMap[i][j].explored) { Renderer.drawRect(roomColors[dungeonMap[i][j].roomType][0], settings.mapX + (i*ms), settings.mapY + (j*ms), ms, ms) }
	...
	if (settings.scoreCalc) {
		...
		let speedScore = 100
		let bonusScore = (dungeonCrypts >= 5 ? 5 : dungeonCrypts) + (mimicFloors.includes(dungeonFloor) && settings.assumeMimic ? 2 : 0) + (settings.ezpzPaul ? 10 : 0)
		...
		let msg1 = `&7Secrets: &a${bbbbb}    &7Crypts: &a${ccccc}`
...
function drawMarker(markerInfo) {
	...
	if (settings.showIconNames) {
		renderCenteredText(markerInfo.name, (markerInfo.iconX+iconDims[0]/2)*2, (markerInfo.iconY+7)*2, 5, false)
	}
...
register("step", () => {
	new Thread(() => {
		if (!renderingMap) { return }
		if (!settings.darkenUnexplored && !settings.showCheckmarks) { return }
		if (inBoss) { return }
		let map
		let mapData
		let mapColors
