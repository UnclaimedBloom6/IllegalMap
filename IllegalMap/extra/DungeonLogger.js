...
class DungeonLogger {
    constructor() {
        ...
        this.exclusions = ["Entrance", "Fairy", "Blood", "Unknown"]
        this.traps = ["New", "Old"]
        ...
        register("tick", () => {
            ...
            if (Dungeon.fullyScanned) {
                ...
                thisLog = this.convertToLog(thisLog)
                this.addLog(thisLog)
            }
        })
    ...
    //     log.r = log.r.map(a => { return dungRooms[a] })
    //     log.p = log.p.map(a => { return dungRooms[a] })
    //     log.t = this.traps[log.t]
...
export default new DungeonLogger()